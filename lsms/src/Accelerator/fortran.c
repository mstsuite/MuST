...
#ifdef ACCELERATOR_LIBSCI
#include "libsci_acc.h"
void zgemm_acc2_ (const char *transa, const char *transb, const int *m,
                   const int *n, const int *k, const cuDoubleComplex *alpha,
                   const devptr_t *devPtrA, const int *lda,
                   const devptr_t *devPtrB, const int *ldb,
                   const cuDoubleComplex *beta, const devptr_t *devPtrC,
...
void zgetrf_acc2_( const int *m, const int *n,
                    const devptr_t *devPtrA, const int *lda,
                    int *ipiv,  int *info )
{
    double *A = (double *)*devPtrA;
    zgetrf_acc(  *m, *n, A, *lda, ipiv, info );
}

void zgetrs_acc2_( const char *transa, const int *m, const int *nrhs,
                    const devptr_t *devPtrA, const int *lda,
                    int *ipiv,  const devptr_t *devPtrB, const int *ldb,
                  int *info )
{
    double *A = (double *)*devPtrA;
    double *B = (double *)*devPtrB;
    zgetrs_acc( *transa, *m, *nrhs, A, *lda,
                 ipiv, B, *ldb, info );
}
#endif
