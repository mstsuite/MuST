...
c     ================================================================
      ...
      use MPPModule, only : waitMessage, syncAllPEs
c
      implicit   none
c
c     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
c
      ...
      data block_size/3/,block_dir/0/
! meis: under some circumstances the blk_sz can exceed the matrix size
! as a temporary fix set block_dir to zero and stick with initial guess
! this will not give the optimal performance!
...
c
      ...
      msgsize=16*mtxsize
c     ================================================================
!     write(6, *) "before allocate in gettaucl_c.bu.f"
      ...
      allocate( bgij(1:kkrsz*kkrsz*n_spin_cant*n_spin_cant) )
      j=max(nrmat*(kkrsz*n_spin_cant*6+6)*n_spin_cant,
     &      nrmat*kkrsz*n_spin_cant*n_spin_cant+kkrsz*kkrsz+
     &      2*lmax+1+ndlj)

      if(iprint .ge. 1) then
        write(6, *) "j, nrmat, kkrsz, n_spin_cant, nrsclu, lmax"
        write(6, *) j, nrmat, kkrsz, n_spin_cant, nrsclu, lmax
        call flush(6)
      endif
...
c              -------------------------------------------------------
               psq = prel*prel
               call setgij(gij,bgij,kkr1, kkr1_ns,kkr2,kkr2_ns,
     >                  lmax,n_spin_cant,nrel_rel,psq,energy)
c              -------------------------------------------------------
c              ====================================================
c                    s,s'  ij
...
c              into wbig => 1-t*G .........................
c              -------------------------------------------------------
               call zgemm('n','n',kkr1_ns,kkr2_ns,kkr1_ns,cmone,
     >                       tmat_n,kkr1_ns,bgij,kkr1_ns,czero,
     &                       wbig(nrmat_ns*ncst+nrst+1),nrmat_ns)
c              -------------------------------------------------------
            endif
...
c     print if needed.................................................
      if(iprint.ge.1) then
         write(6,'('' gettaucl:: 1-t*g(Rij) :: mynod ='',1i5)') mynod
         call wrtmtx(wbig,nrmat_ns,istop)
      endif
c     ================================================================
...
!     if(istop.eq.sname) then
        if(iprint.ge.1) then
          write(6,*) 'Writing zblock.data'
          call zblock_lu_writer(wbig,nrmat_ns,blk_sz,nblk)
        end if
!     end if
...
c     ================================================================
      ...
      deallocate( msgids, bgij )
c     ================================================================
      if (istop.eq.sname) then
         write(6,*) 'TAU00_lms'
         if (nrel_rel.ne.0) then
           call clebsch
           call lmsmtrx(tmat_n,tau00,kkrsz_ns)
           call outmat1(tmat_n,kkrsz_ns,kkrsz_ns,kkrsz_ns,1.d-15,6)
         else
           call outmat1(tau00,kkrsz_ns,kkrsz_ns,kkrsz_ns,1.d-15,6)
         end if
         call syncAllPEs()
         call fstop(sname)
      else
