...
int main(int argc, char *argv[])
{
  ...
  if(argc>1 && *argv[1]=='c')
  {
    ...
    t0 = omp_get_wtime();
    buildKKRMatrix_kokkos(lsms, local, atom, energy, prel,m);
    t1 = omp_get_wtime();
    printf("Running new version ");
    ...
    t1 = omp_get_wtime();
    printf(" time: %lf seconds.\n", t1-t0);
    // buildKKRMatrix_nrel_ns2(lsms, local, atom, energy, prel,m2);
  ...
  } else if(argc>1 && *argv[1]=='C')
  {
    ...
    printf("Comparing matrices\n");
    m1.resize(atom.nrmat*lsms.n_spin_cant,atom.nrmat*lsms.n_spin_cant);
    m2.resize(atom.nrmat*lsms.n_spin_cant,atom.nrmat*lsms.n_spin_cant);
    // buildKKRMatrix_kokkos(lsms, local, atom, energy, prel,m);
    buildKKRMatrix_orig(lsms, local, atom, energy, prel,m1);
    buildKKRMatrix_nrel_ns2(lsms, local, atom, energy, prel,m2);
    for(int i=0; i<atom.nrmat*lsms.n_spin_cant; i++)
      for(int j=0; j<atom.nrmat*lsms.n_spin_cant; j++)
      {
        Real d=std::abs(m1(i,j)-m2(i,j));
        Real rd=0.0;
        if(m1(i,j)!=m2(i,j)) rd=d/std::abs(0.5*(m1(i,j)+m2(i,j)));
        if(rd>1.0e-10) printf("m1(i,j)!=m2(i,j): i=%d, j=%d, m1(i,j)=(%lg,%lg), m2(i,j)=(%lg,%lg) d=%lg rd=%lg\n",
                                   i,j,std::real(m1(i,j)),std::imag(m1(i,j)),std::real(m2(i,j)),std::imag(m2(i,j)),
                                   d,rd);
      }
