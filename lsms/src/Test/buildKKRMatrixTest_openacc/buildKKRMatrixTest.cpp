...
int main(int argc, char *argv[])
{
  ...
  for(int i=0; i<32*32; i++) local.tmatStore(i,0)=tmat[i];
  ...
  if(argc>1 && *argv[1]=='c')
  {
    ...
    t0 = omp_get_wtime();
    buildKKRMatrix_nrel_ns2_acc(lsms, local, atom, energy, prel,m1);
    t1 = omp_get_wtime();
    ...
    if ( isnan(abs(m(i,j))) || isnan(abs(m1(i,j))) || fabs(m(i,j)-m1(i,j))>1e-10 )
    {
      printf("m(i,j)!=m1(i,j): i=%d, j=%d, m(i,j)=(%lg,%lg), m1(i,j)=(%lg,%lg) d=%lg rd=%lg\n",
    ...
    {
      buildKKRMatrix_nrel_ns2_acc(lsms, local, atom, energy, prel,m);
    }
    t1 = omp_get_wtime();
    printf("OpenACC time: %f\n",t1-t0);
  } else {
