...
!           ----------------------------------------------------------
            call solveSingleScattering(is, id, energy, CZERO,         &
                                       useIrrSol='H', isCheckWronsk = .true.)
!           ----------------------------------------------------------
         enddo
         write(6,'(a,f12.5,a)') 'Compute time in solveSingleScattering: ',getTime() - t2,' sec.'
      enddo
   enddo
!
   if (getMyPEinGroup(eGID) == 0) then
