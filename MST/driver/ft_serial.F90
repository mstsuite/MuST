program ft_serial
!  ********************************************************************
!  test FFTW serial fast Fourier transform routines using Fortran 2003
!
...
!
   complex (kind=CmplxKind), allocatable :: ft_fftwc(:)
!
   complex (kind=CmplxKind), allocatable :: ft_fftw(:)
!
   complex (kind=CmplxKind), allocatable :: ft_fftwr(:)
!
...
#ifdef FFTW
!  ===================================================================
!  Allocate space and initialize functions 
!  ===================================================================
   allocate(rfunc(nt))
   allocate(ft_fftw(nt))
   allocate(ft_fftwr((nx/2+1)*ny*nz))
   allocate(ft_fftwc(1:nt))
!
   c_nx = nx; c_ny = ny; c_nz = nz; c_nt = nt
   c_func_fftwr = fftw_alloc_real((c_nx+2)*c_ny*c_nz)
   call c_f_pointer(c_func_fftwr, rfunc_fftwr, [(c_nx+2)*c_ny*c_nz])
   call c_f_pointer(c_func_fftwr, cfunc_fftwr, [(c_nx/2+1)*c_ny*c_nz])
!
   allocate( kvec_fftw(3,nt), ft_fftw_ip(nt) )
!
   idr = 0
   do k = 1, nz
      z = (k-1)*dz
      do j = 1, ny
         ...
         do i = 1, nx
            x = (i-1)*dx
            idr = idr + 1
            rfunc(idr) = exp(alpha*x+beta*y+gamma*z)
            ft_fftwc(idr) = cmplx(rfunc(idr),ZERO,kind=C_DOUBLE_COMPLEX)
!
...
!
!  ===================================================================
!  Test FFTW complex -> complex, store the results in ft_fftwc_o
!  ===================================================================
   t0 = getTime()
!  -------------------------------------------------------------------
   plan = fftw_plan_dft_3d(  nz, ny, nx, ft_fftwc, ft_fftwc,          &
                             FFTW_BACKWARD, FFTW_ESTIMATE )
   call fftw_execute_dft( plan, ft_fftwc, ft_fftwc )
   call fftw_destroy_plan( plan )
   ft_fftwc = ft_fftwc/vfac
!  -------------------------------------------------------------------
   write(6,'('' time for fftw cmplx ='',1f10.5)')getTime()-t0
!
!  ===================================================================
!  Test FFTW out-place real -> complex, store the results in ft_fftwr
!  ===================================================================
!
   t0 = getTime()
!  -------------------------------------------------------------------
   plan = fftw_plan_dft_r2c_3d( nz, ny, nx, rfunc, ft_fftwr, FFTW_ESTIMATE )
   call fftw_execute_dft_r2c( plan, rfunc, ft_fftwr )
   call fftw_destroy_plan( plan )
   ft_fftwr = ft_fftwr/vfac
!  -------------------------------------------------------------------
   write(6,'('' time for fftw real->complex (R2C) ='',1f10.5)')getTime()-t0
!
!  ===================================================================
!  Test FFTW in-place real -> complex, store the results in rfunc_fftwr
!  ===================================================================
   t0 = getTime()
!  -------------------------------------------------------------------
   plan = fftw_plan_dft_r2c_3d( nz, ny, nx, rfunc_fftwr, cfunc_fftwr, &
                                FFTW_ESTIMATE )
   call fftw_execute_dft_r2c( plan, rfunc_fftwr, cfunc_fftwr )
   call fftw_destroy_plan( plan )
   rfunc_fftwr = rfunc_fftwr/vfac
!  -------------------------------------------------------------------
   write(6,'('' time for in-place fftw real->complex (R2C) ='',1f10.5)')getTime()-t0
!
!  ===================================================================
   ...
   do idk = 1, nt
      ...
      LOOP_idk_w: do idm = 1, nt
         if ( abs(kvec_fftw(1,idk)-kvec_nr(1,idm)) < TEN2m6 .and.     &
            ...
            if (abs(ft_nr(idm)-ft_fftwc(idk)) > TEN2m6 .or.             &
                abs(ft_nr(idm)-ft_fftw(idk)) > TEN2m6 .or.              &
                abs(ft_nr(idm)-ft_fftw_ip(idk)) > TEN2m6) then
               write(6,'(a)')'ERROR: bad transformation'
               write(6,'(3f10.5,4(2x,2d15.8))')kvec_nr(1:3,idm), ft_nr(idm), ft_fftwc(idk), &
                                               ft_fftw(idk), ft_fftw_ip(idk)
               stop 'Error'
            else
               write(6,'(3f10.5,4(2x,2d15.8))')kvec_nr(1:3,idm), ft_nr(idm), ft_fftwc(idk), &
                                               ft_fftw(idk), ft_fftw_ip(idk)
      ...
      enddo LOOP_idk_w
      if (.not. found) then
         write(6,'(a)')'k-vectors can not be matched in FFTW case!'
         stop 'ERROR'
      endif
   enddo
!
   write(6,'(a)')'Serial FFTW test is successful!'
!
   deallocate( kvec_fftw, rfunc, ft_fftw, ft_fftw_ip, ft_fftwr, ft_fftwc )
