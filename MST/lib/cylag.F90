!     cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      function cylag(xi,x,y,ind1,n1,imax,iex) result(lag)
!     ================================================================
!
...
!     y is array from which cylag is obtained by interpolation
...
!
      real (kind=RealKind) :: dx(nmax)
      real (kind=RealKind) :: dift, dif, den
!
      complex (kind=CmplxKind) :: c(nmax), d(nmax)
      complex*16 lag, cden
!
      ind=ind1
      n=n1
...
!        if (xi-x(j)) 30,130,20
   ...
   80 dif=abs(xi-x(inl+1))
      ...
      do j=inl+1,inu-1
	    ...
	    ij=i+j
            den=x(i+inl)-x(ij)
            if(den.eq.0.d0) stop 'Two xs are the same in cylag.'
            cden=(d(i)-c(i+1))/den
            d(i)=dx(ij-inl)*cden
            c(i)=dx(i)*cden
         enddo
