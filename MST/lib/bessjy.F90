...
!
   real (kind=Realkind), intent(in) :: x,xnu
   real (kind=Realkind), intent(out) :: rj,rjp,ry,ryp
...
1  continue
   ...
   if(x.lt.XMIN) then
      ...
      do i=1,MAXIT
         ...
         del1=c*p-i*del
         sum1=sum1+del1
         if (abs(del).lt.(one+abs(sum))*EPS) then
            goto 2
         endif
      enddo
...
2     continue
      ...
      do i=2,MAXIT
         ...
         q=p*dli+q*dlr
         p=temp
         if(abs(dlr-1.d0)+abs(dli).lt.EPS) then
            goto 3
         endif
      enddo
