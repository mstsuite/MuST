...
!
public :: initMatsubaraApprox,        &
          endMatsubaraApprox,         &
          getNumMatsubaraApproxPoles, &
          calMatsubaraApproxPoles
!
private
!
...
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine initMatsubaraApprox(eGrid,Temp,eWidth,NumPs,iprint)
!  ===================================================================
...
!
   if (Temp < ZERO) then
!     ----------------------------------------------------------------
      call ErrorHandler('initMatsubaraApprox','Temperature < 0',Temp)
!     ----------------------------------------------------------------
   else if (present(NumPs)) then
      if (NumPs < 1) then
!        -------------------------------------------------------------
         call ErrorHandler('initMatsubaraApprox','Number energy points < 1',NumPs)
!        -------------------------------------------------------------
      endif
...
!           ----------------------------------------------------------
            call ErrorHandler('initMatsubaraApprox',                  &
...
!     ----------------------------------------------------------------
      call ErrorHandler('initMatsubaraApprox','Invalid e-grid type',eGrid)
!     ----------------------------------------------------------------
   endif
!
   PoleType = eGrid
   Initialized = .true.
!
   end subroutine initMatsubaraApprox
!  ===================================================================
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine endMatsubaraApprox()
!  ===================================================================
   implicit none
!
   NumPoles = 0
   ...
   Initialized = .false.
!
   end subroutine endMatsubaraApprox
!  ===================================================================
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   function getNumMatsubaraApproxPoles() result(n)
!  ===================================================================
   implicit none
!
...
!
   if (.not.Initialized) then
      call ErrorHandler('getNumMatsubaraApproxPoles',                 &
                        'MatsubaraApproxModule is not initialized')
   endif
!
   n = NumPoles
!
   end function getNumMatsubaraApproxPoles
!  ===================================================================
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine calMatsubaraApproxPoles(ebot,etop,epole,weight,core_top)
!  ===================================================================
...
!
   if (.not.Initialized) then
      call ErrorHandler('calMatsubaraApproxPoles',                    &
                        'MatsubaraApproxModule is not initialized')
   else if (size(epole) < NumPoles) then
      call ErrorHandler('calMatsubaraApproxPoles',                    &
                        'size of epole array < NumPoles',             &
                        size(epole),NumPoles)
   else if (size(weight) < NumPoles) then
      call ErrorHandler('calMatsubaraApproxPoles','size of weight array < NumPoles',&
...
!
   end subroutine calMatsubaraApproxPoles
!  ===================================================================
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
!                                 |
!                               x | x Im(e)
!                           x     |     x       Matsubara Approximant poles
!                       x         |         x
!                     x           |           x
!                   x             |             x
!                  x              |              x
...
!  nume        : total number of generated Matsubara approximant energies.
...
!  ==================================================================
end module MatsubaraApproxModule
