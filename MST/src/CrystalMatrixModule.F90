...
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   function calSigmaIntegralSRO(n,e,ic1,ic2,Ja,Jb,getSingleScatteringMatrix,&
...
!  ===================================================================
   ...
   use GroupCommModule, only : getGroupID, GlobalSumInGroup, getMyPEinGroup
   use StrConstModule, only : getStrConstMatrix
   use SROModule, only : clusterDtilde
   use StrConstModule, only : checkFreeElectronPoles, getFreeElectronPoleFactor
   use WriteMatrixModule,  only : writeMatrix
...
! 
   ...
   logical :: calculate_tau = .false.
! 
   character (len=20) :: sname = "calSigmaIntegralSRO"
! 
   integer (kind=IntKind), intent(in) :: n, ic1, ic2, caltype
   integer (kind=IntKind), intent(in), optional :: configuration(:)
   integer (kind=IntKind) :: k_loc, k, row, col, MyPEinKGroup, method
   integer (kind=IntKind) :: NumKs, kGID, aGID, NumKsOnMyProc, NumRedunKs
   integer (kind=IntKind) :: site_config(LocalNumAtoms), itertmp
...
! 
   ...
   complex (kind=CmplxKind), allocatable :: tmbsym(:,:), tmb(:,:), tau1(:,:), tau2(:,:)
   complex (kind=CmplxKind), allocatable :: D1(:,:), Dt1(:,:), D2(:,:), Dt2(:,:)
   complex (kind=CmplxKind) :: prod1(kmax_kkr_max, kmax_kkr_max), &
     prod2(kmax_kkr_max, kmax_kkr_max), prod3(kmax_kkr_max, kmax_kkr_max), &
     prod4(kmax_kkr_max, kmax_kkr_max), prod5(kmax_kkr_max, kmax_kkr_max), &
     prod6(kmax_kkr_max, kmax_kkr_max), tilde(kmax_kkr_max, kmax_kkr_max)
! 
   interface
      function getSingleScatteringMatrix(smt,spin,site,atom,dsize) result(sm)
         use KindParamModule, only : IntKind, CmplxKind
...
!  ===================================================================
   do k_loc = 1,NumKsOnMyProc
     D1 = CZERO; Dt1 = CZERO; D2 = CZERO; Dt2 = CZERO
     tilde = CZERO; prod1 = CZERO; prod2 = CZERO
     prod3 = CZERO; prod4 = CZERO; prod5 = CZERO; prod6 = CZERO
!    ================================================================
...
!  -------------------------------------------------------------------
   ...
   end function calSigmaIntegralSRO
!  ===================================================================
!
!  *******************************************************************
!
...
! 
   ...
   logical :: calculate_tau = .false.
! 
   character (len=20) :: sname = "calSigmaIntegralCPA"
! 
